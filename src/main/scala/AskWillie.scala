...
@main def main(): Unit = {
        ...
        val pageRank = PageRank.pagerank(pages)
        ...
        while {
          ...
          val searchedPages: List[SearchedWebPage] = (for i <- rankedPages.indices yield SearchedWebPage(rankedPages(i), pageCount(i))).toList // call PageSearch.???? here
